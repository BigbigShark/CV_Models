...
from .feature_pyramid import BackboneWithFPN, LastLevelMaxPool
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=10, include_top=True, norm_layer=None):
        super().__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu') # 用服从正态分布的随机矩阵进行初始化
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        for _ in range(1, block_num): # _: temp variable
...
def resnet50_fpn_backbone(pretrain_path="",
    ...
    Args:
        ...
        norm_layer: FrozenBatchNorm2d，当batch_size比较小时，mean和var不那么具有稳定性和代表性，还不如不使用BN层
    ...
    if isinstance(norm_layer, FrozenBatchNorm2d):
        overwrite_eps(resnet_backbone, 0.0) # 将eps设为0
    ...
    assert 0 <= trainable_layers <= 5, "trainable_layers {} not between 0 and 5".format(trainable_layers)
    ...
    # 返回的特征层个数肯定大于0小于5
    assert min(returned_layers) > 0 and max(returned_layers) < 5, "returned_layers wrong."
